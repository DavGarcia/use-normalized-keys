...
/**
 * Hook for tracking hold progress of a specific sequence
 * Provides smooth animated progress values without manual animation frame management
 * 
...
export function useHoldProgress(sequenceId: string) {
  ...
  // Get current hold to trigger effect when it changes
  ...
  useEffect(() => {
    const updateProgress = () => {
      ...
      if (currentHold) {
        ...
        const elapsed = Date.now() - currentHold.startTime;
        const targetProgress = Math.min(100, (elapsed / currentHold.minHoldTime) * 100);
        
        // Smooth interpolation
        const currentProgress = lastProgressRef.current + (targetProgress - lastProgressRef.current) * 0.15;
      ...
      } else if (lastProgressRef.current > 0.1) {
        // Animate back to zero when released
        const currentProgress = lastProgressRef.current * 0.85;
        lastProgressRef.current = currentProgress;
        setSmoothProgress(currentProgress);
        animationFrameRef.current = requestAnimationFrame(updateProgress);
      } else {
    ...
    // Start animation if hold exists
    if (hold) {
      animationFrameRef.current = requestAnimationFrame(updateProgress);
    }
...
/**
 * Hook for animated hold progress with visual properties
 * Provides smooth animations for progress bars, scales, opacity, etc.
 * 
 * @param sequenceId - The ID of the hold sequence to track
...
/**
 * Game-oriented hook for tracking sequence state with event detection
 * Provides easy-to-use flags for game logic like justStarted, justCompleted, etc.
 * 
 * @param sequenceId - The ID of the sequence to track
...
export function useSequence(sequenceId: string) {
  ...
  // Track hold started/cancelled events
  useEffect(() => {
  ...
  return {
    ...
    // Event flags (useful for triggering one-time actions)
